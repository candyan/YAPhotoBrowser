    ...
    [UIView animateWithDuration:_animationDuration
        animations:^{
            ...
            resizableImageView.frame =
                CGRectMake(0, (screenHeight / 2) - ((imageFromView.size.height / scaleFactor) / 2), screenWidth,
...
- (void)_performCloseAnimationWithScrollView:(YAPhotoZoomingScrollView *)scrollView
{
    ...
    UIImageView *resizableImageView = [[UIImageView alloc] initWithImage:imageFromView];
    resizableImageView.frame =
        (imageFromView) ? CGRectMake(0, (screenHeight / 2) - ((imageFromView.size.height / scaleFactor) / 2) +
                                            scrollView.frame.origin.y,
                                     screenWidth, imageFromView.size.height / scaleFactor)
                        : CGRectZero;
    resizableImageView.contentMode = UIViewContentModeScaleAspectFill;
...
- (UILabel *)pagesLabel
{
    if (!_pagesLabel && self.totalPages > 1) {
...
- (void)setInitialPageIndex:(NSUInteger)pageIndex
{
    ...
    if (self.totalPages > 1 && [self isViewLoaded]) {
        [self _setupPagesTip];
        [self.pageScrollView setContentOffset:[self _contentOffsetForPageAtIndex:pageIndex] animated:YES];
    }
}
...
- (void)setShowPagesTip:(BOOL)showPagesTip animated:(BOOL)flag
{
    ...
    if ([self isViewLoaded]) {
        ...
        [UIView animateWithDuration:duration
            animations:^{ [self.pagesLabel setAlpha:showPagesTip ? 1.0f : 0.0f]; }
            completion:^(BOOL finished) {
...
- (void)_displayPages
{
    ...
    NSInteger iFirstIndex =
        floorf((CGRectGetMinX(visibleBounds) + kScrollPagePadding * 2) / CGRectGetWidth(visibleBounds));
    NSInteger iLastIndex =
        floorf((CGRectGetMaxX(visibleBounds) - kScrollPagePadding * 2 - 1) / CGRectGetWidth(visibleBounds));
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self _isDisplayingPageForIndex:index]) {
            ...
            if (self.progressTintColor)
                page.progressView.progressColor = self.progressTintColor;
...
{
    [self.pagesLabel
        setText:[NSString stringWithFormat:@"%u / %lu", self.currentPageIndex + 1, (unsigned long)self.totalPages]];
}
...
- (CGSize)_contentSizeForPageScrollView
{
    ...
    CGSize scrollContentSize =
        CGSizeMake(pageScrollViewFrame.size.width * self.totalPages, pageScrollViewFrame.size.height);
...
- (void)photoZoomingScrollView:(YAPhotoZoomingScrollView *)scrollView singleTapDetected:(UITapGestureRecognizer *)tapGR
{
